...
from .models import YouTubeVideo, OpenAIArticle, AnthropicArticle, GeminiArticle, DeepSeekArticle, Digest
...
class Repository:
    ...
    def create_gemini_article(self, guid: str, title: str, url: str, published_at: datetime,
                              description: str = "", category: Optional[str] = None) -> Optional[GeminiArticle]:
        existing = self.session.query(GeminiArticle).filter_by(guid=guid).first()
        if existing:
            return None
    ...
    def create_deepseek_article(self, guid: str, title: str, url: str, published_at: datetime,
                                description: str = "", category: Optional[str] = None) -> Optional[DeepSeekArticle]:
        existing = self.session.query(DeepSeekArticle).filter_by(guid=guid).first()
        if existing:
            return None
        article = DeepSeekArticle(
            guid=guid,
            title=title,
            url=url,
            published_at=published_at,
    ...
    def bulk_create_gemini_articles(self, articles: List[dict]) -> int:
        new_articles = []
        for a in articles:
            existing = self.session.query(GeminiArticle).filter_by(guid=a["guid"]).first()
            if not existing:
                new_articles.append(GeminiArticle(
                    guid=a["guid"],
                    title=a["title"],
                    url=a["url"],
                    published_at=a["published_at"],
    ...
    def bulk_create_deepseek_articles(self, articles: List[dict]) -> int:
        new_articles = []
        for a in articles:
            existing = self.session.query(DeepSeekArticle).filter_by(guid=a["guid"]).first()
            if not existing:
                new_articles.append(DeepSeekArticle(
                    guid=a["guid"],
                    title=a["title"],
                    url=a["url"],
                    published_at=a["published_at"],
